...
class StateMachineValidator:
                ...
                if action_type in self.STANDARD_PATTERNS:
                    ...
                    for expected_event in expected_events:
                        has_transition = any(
                            ...
                            and t.get("event") == expected_event
                            for t in transitions
                        )

                        if not has_transition and expected_event not in [
                            "in_expected_state",
                            "unexpected_state",
                            "not_running",
                        ]:
    ...
    def _check_orphaned_states(
        ...
        orphaned = (
            states - target_states - {"stopped"}
        )  # 'stopped' is terminal, OK to be orphaned
    ...
    def _check_unreachable_states(
        ...
        for state in unreachable:
            if (
                state != "stopped"
            ):  # Terminal state, OK to be unreachable via normal flow
                result.add_warning(
                    ValidationIssue(
...
Examples:
  ...
  statemachine-validate --quiet config/*.yaml
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Only show errors, suppress warnings"
    )
    parser.add_argument(
    ...
    total_errors, total_warnings = print_results(
        results, quiet=args.quiet, use_color=not args.no_color
    )
