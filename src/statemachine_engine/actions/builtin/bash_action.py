    ...
    async def execute(self, context: dict[str, Any]) -> str:
        ...
        if not command:
            ...
            if command and job and isinstance(job.get("data"), dict):
                ...
                context_data = {
                    **context
                }  # Start with full context (includes propagated job fields)
                    ...
                    if value is not None:
                        ...
                        if quoted_placeholder in command:
                            ...
                            command = command.replace(
                                quoted_placeholder, f"'{escaped_value}'"
                            )
                            machine_name = self.get_machine_name(context)
                            logger.debug(
                                f"[{machine_name}] Substituted {quoted_placeholder} with '{escaped_value}' (using {primary_key if primary_key in context_data else fallback_key})"
                            )
                        ...
                        if placeholder in command:
                            ...
                            machine_name = self.get_machine_name(context)
                            logger.debug(
                                f"[{machine_name}] Substituted {placeholder} with {quoted_value} (using {primary_key if primary_key in context_data else fallback_key})"
                            )
                ...
                for key, value in context_data.items():
                    if key != "event":  # Skip event field
                        ...
                        if quoted_placeholder in command:
                            ...
                            machine_name = self.get_machine_name(context)
                            logger.debug(
                                f"[{machine_name}] Substituted {quoted_placeholder} with '{escaped_value}'"
                            )
                        ...
                        if placeholder in command:
                            ...
                            machine_name = self.get_machine_name(context)
                            logger.debug(
                                f"[{machine_name}] Substituted {placeholder} with {quoted_value}"
                            )
                ...
                if remaining_placeholders:
                    machine_name = self.get_machine_name(context)
                    logger.warning(
                        f"[{machine_name}] Unsubstituted placeholders found: {remaining_placeholders}"
                    )

        if not command:
            machine_name = self.get_machine_name(context)
            logger.error(
                f"[{machine_name}] No command specified in job data or bash action config"
            )
            return "error"
        ...
        if not command.startswith("mkdir -p"):
            ...
            if self.__class__._cmd_count[cmd_key] == 1:
                logger.info(
                    f"[{machine_name}] [BASH FREQUENCY TRACKER] First execution: {command}"
                )
            elif self.__class__._cmd_count[cmd_key] % 100 == 0:
                logger.warning(
                    f"[{machine_name}] [BASH FREQUENCY TRACKER] Command executed {self.__class__._cmd_count[cmd_key]} times: {command}"
                )

            logger.debug(
                f"[{machine_name}] Executing command for job {job_id}: {command}"
            )
        ...
        try:
            ...
            if stdout_text and not is_routine:
                logger.debug(
                    f"[{machine_name}] Output: {stdout_text[:200]}"
            ...
            if process.returncode == 0:
                ...
                logger.debug(
                    f"[{machine_name}] Command completed (exit 0): {cmd_summary}"
                )
                ...
                logger.error(
                    f"[{machine_name}] Command failed with exit code {process.returncode} for job {job_id}"
                )
                ...
                context["last_error"] = (
                    f"Command failed (exit {process.returncode}): {error_output}\nCommand: {command}"
                )
                ...
                if str(process.returncode) in error_mappings:
                    ...
                    logger.info(
                        f"[{machine_name}] Mapping exit code {process.returncode} to event: {mapped_error}"
                    )
