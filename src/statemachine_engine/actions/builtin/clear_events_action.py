    ...
    async def execute(self, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not self.event_types:
                logger.warning(
                    f"[{machine_name}] No event types specified for clearing"
                )
                return self.get_config_value("success", "no_events_to_clear")

            if not self.target_machine:
                logger.warning(
                    f"[{machine_name}] No target machine specified for clearing events"
                )
                return self.get_config_value("success", "no_events_to_clear")
            ...
            for event in all_events:
                if event["event_type"] in self.event_types:
                    ...
                    total_cleared += 1
                    logger.debug(
                        f"Cleared pending event: {event['event_type']} (ID: {event['id']})"
                    )

            if total_cleared > 0:
                logger.info(
                    f"[{machine_name}] Cleared {total_cleared} pending events of types: {', '.join(self.event_types)}"
                )
                logger.info(f"[{machine_name}] Target machine: {self.target_machine}")
                return self.get_config_value("success", "events_cleared")
            else:
                logger.debug(
                    f"[{machine_name}] No pending events found for types: {', '.join(self.event_types)}"
                )
                return self.get_config_value("success", "no_events_to_clear")
