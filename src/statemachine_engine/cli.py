...
async def run_state_machine(
    ...
    engine = StateMachineEngine(
        ...
        control_socket_prefix=control_socket_prefix,
    )
...
async def async_main():
    ...
    parser.add_argument(
        "--machine-name", help="Override machine name (default: read from config)"
    )
    parser.add_argument(
        "--actions-dir", help="Custom actions directory (absolute or relative path)"
    )
    parser.add_argument(
        "--event-socket-path",
        help="Custom event socket path (default: /tmp/statemachine-events.sock)",
    )
    parser.add_argument(
        "--control-socket-prefix",
        help="Custom control socket prefix (default: /tmp/statemachine-control)",
    )
    parser.add_argument(
        "--initial-context",
        default="{}",
