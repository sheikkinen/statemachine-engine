    ...
    for transition in stop_transitions:
        ...
        if from_state == "*":
            ...
            active_states = [
                s
                for s in states
                if isinstance(s, str) and s not in ["stopped", "error_cleanup"]
            ]
            ...
            for line in f:
                ...
                if in_states_section:
                    ...
                    if "# ===" in line and "===" in line:
                        ...
                        group_name = (
                            comment.replace("===", "").strip().replace(" STATES", "")
                        )
    ...
    for group_name, group_states in state_groups.items():
        ...
        for t in transitions:
            if (
                ...
                and state_to_group.get(t.get("from")) != group_name
            ):
...
def generate_markdown(config: dict[str, Any], yaml_path: str) -> str:
    ...
    if error_diagram:
        md_content.extend(["## Error Handling Flow", "", error_diagram, "", "---", ""])

    # Stop Handling Diagram
    stop_diagram = generate_stop_handling_diagram(config)
    if stop_diagram:
        md_content.extend(["## Stop/Shutdown Flow", "", stop_diagram, "", "---", ""])
...
def generate_composite_subdiagram(
    ...
    lines.append("    %% Internal transitions")
    ...
    lines.append("    %% External composites")
    for other_group_name in all_groups:
        if other_group_name != group_name:
            other_composite = (
                other_group_name.replace(" ", "").replace("STATES", "").strip()
            )
    ...
    lines.append("    %% Transitions to other composites")
...
def generate_diagram_files(
    ...
    if not state_groups:
        print(
            f"⚠️  No state groups found in {yaml_path}. Skipping new format generation."
        )
        return

    # Generate main overview diagram
    print("  Generating main.mermaid...")
    ...
    for group_name, group_states in state_groups.items():
        ...
        subdiagram = generate_composite_subdiagram(
            config, group_name, group_states, state_groups
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate FSM diagrams from YAML configuration"
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="docs/fsm-diagrams",
        help="Output directory for new format (default: docs/fsm-diagrams)",
    )
    parser.add_argument(
        "--old-format-only",
        action="store_true",
        help="Generate only old Markdown format",
    )
    parser.add_argument(
        "--new-format-only",
        action="store_true",
        help="Generate only new .mermaid format",
    )
