...
class SendEventAction(BaseAction):
    ...
    async def execute(self, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if self._send_via_socket(payload, job_id, machine_name):
                logger.info(
                    f"[{machine_name}] Sent {self.event_type} to {self.target_machine} via Unix socket"
                )
            else:
                ...
                logger.info(
                    f"[{machine_name}] Sent {self.event_type} to {self.target_machine} via database (event_id: {event_id})"
                )
    ...
    def _send_via_socket(
        ...
        try:
            ...
            if not Path(socket_path).exists():
                logger.debug(
                    f"[{machine_name}] Control socket not found: {socket_path}"
                )
                return False
            ...
            # Log sent message
            logger.info(
                f"[{machine_name}] 📤 Sent event {self.event_type} to {self.target_machine}"
            )
            logger.debug(f"[{machine_name}] 📤 Event payload: {payload}")
        ...
        for key, value in template.items():
            if isinstance(value, str):
                ...
                if (
                    ...
                    machine_name = context.get("machine_name", "unknown")
                    logger.warning(
                        f"[{machine_name}] Variable '{value}' not found in context, using None"
                    )
                    processed[key] = None
                ...
                # Check for recursive {id} in extracted values
                    ...
                    processed[key] = processed[key].replace(
                        "{id}", substitute_id if substitute_id else "{id}"
                    )
            else:
                processed[key] = value
