    ...
    async def execute(self, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            job = self.job_model.get_next_job(
                job_type=self.job_type, machine_type=self.machine_type
            )
            ...
            if job:
                ...
                logger.info(
                    f"[{machine_name}] {job['job_type']} job {job['job_id']} retrieved from queue (data keys: {list(job_data.keys())})"
                )
        ...
        try:
            ...
            for job in problem_jobs:
                ...
                if input_path is None:
                    ...
                    logger.warning(
                        f"[{machine_name}] Skipping job {job_id} - no input image path"
                    )
                    continue
                ...
                machine_name = self.config.get("machine_type", "unknown")
                logger.warning(
                    f"[{machine_name}] Marking job {job_id} as failed - {reason}"
                )
            ...
            if problem_jobs:
                machine_name = self.config.get("machine_type", "unknown")
                logger.info(
                    f"[{machine_name}] Marked {len(problem_jobs)} jobs with missing input files as failed"
                )
