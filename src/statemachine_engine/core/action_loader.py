...
class ActionLoader:
    ...
    def __init__(self, actions_root: str = None):
        ...
        self.custom_actions_root = (
            Path(actions_root).resolve() if actions_root else None
        )
    ...
    def _discover_action_modules(self) -> None:
        ...
        if self.builtin_actions_root.exists():
            builtin_count = self._discover_from_directory(
                self.builtin_actions_root, is_custom=False
            )
        else:
            logger.warning(
                f"Built-in actions root not found: {self.builtin_actions_root}"
            )
        ...
        if self.is_custom_root:
            if self.custom_actions_root.exists():
                custom_count = self._discover_from_directory(
                    self.custom_actions_root, is_custom=True
                )
            else:
                logger.warning(
        ...
        for action_file in root_dir.rglob("*_action.py"):
            ...
            if is_custom:
                ...
                if action_type not in self._action_map:
                    ...
                    module_path = "statemachine_engine." + str(
    ...
    def load_action_class(self, action_type: str) -> Optional[type]:
        ...
        if resolved_action_type not in self._action_map:
            logger.warning(
                f"Action type '{action_type}' not discovered. Available: {list(self._action_map.keys())}"
            )
            return None
        ...
        try:
            ...
            is_file_path = (
                ...
                if "/" in path_or_module or "\\" in path_or_module
                else False
            )
            ...
            if is_file_path:
                ...
                spec = importlib.util.spec_from_file_location(
                    f"custom_action_{resolved_action_type}", path_or_module
                )
                if spec is None or spec.loader is None:
                    raise ImportError(
                        f"Failed to create module spec for {path_or_module}"
                    )
            ...
            if len(self._class_cache) <= len(
                self._action_map
            ):  # Still loading initial set
