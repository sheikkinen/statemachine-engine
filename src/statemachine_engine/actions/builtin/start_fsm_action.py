...
class StartFsmAction(BaseAction):
    ...
    async def execute(self, context: dict[str, Any]) -> str:
        ...
        try:
            ...
            # Build command
            command = ["statemachine", yaml_path, "--machine-name", machine_name]
                ...
                if context_data:
                    try:
                        ...
                        if len(context_json) > 4096:
                            logger.warning(
                                f"StartFsmAction: Context JSON is large ({len(context_json)} bytes)"
                            )
                        ...
                        logger.error(
                            f"StartFsmAction: Failed to serialize context: {e}"
                        )
                        return self.get_config_value("error", "error")
            ...
            logger.info(
                f"StartFsmAction: Spawning FSM with command: {' '.join(command)}"
            )
    ...
    def _extract_context_vars(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for var_spec in self.context_vars:
            ...
            if value is not None:
                ...
                logger.warning(
                    f"StartFsmAction: Context variable '{source}' not found, skipping"
                )
