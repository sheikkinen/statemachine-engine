...
def cmd_cleanup(args):
    ...
    if args.status:
        if args.status == "processing":
            ...
            print(
                "WARNING: Use 'reset-processing' command to reset stuck processing jobs to pending."
            )
    ...
    try:
        if args.status:
            ...
            with event_model.db._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM machine_events WHERE status = ?",
                    (args.status,),
                )
                ...
                if count > 0:
                    cursor.execute(
                        "DELETE FROM machine_events WHERE status = ?", (args.status,)
                    )
                    conn.commit()
...
def cmd_list_events(args):
    ...
    try:
        events = event_model.list_events(
            target_machine=args.target, status=args.status, limit=args.limit
        )
        ...
        for event in events:
            ...
            payload = (
                (event["payload"][:30] + "...")
                if event["payload"] and len(event["payload"]) > 30
                else event["payload"]
            )
            rows.append(
                [
...
def cmd_machine_status(args):
    ...
    if args.machine:
        pending_events = events_model.list_events(
            target_machine=args.machine, status="pending"
        )
        processed_events = events_model.list_events(
            target_machine=args.machine, status="processed", limit=5
        )
        print(
            f"  {args.machine}: {len(pending_events)} pending, {len(processed_events)} recent processed"
        )
    else:
        # Check for each machine type
        for machine in ["sdxl_generator", "face_processor"]:
            pending_events = events_model.list_events(
                target_machine=machine, status="pending"
            )
            processed_events = events_model.list_events(
                target_machine=machine, status="processed", limit=5
            )
            print(
                f"  {machine}: {len(pending_events)} pending, {len(processed_events)} recent processed"
            )
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            if proc.info["cmdline"] and "state_machine/cli.py" in " ".join(
                proc.info["cmdline"]
            ):
                cmdline = " ".join(proc.info["cmdline"])
                if "sdxl_generator" in cmdline:
                    running_machines.append(
                        f"  SDXL Generator (PID: {proc.info['pid']}) ✅"
                    )
                elif "face_processor" in cmdline:
                    running_machines.append(
                        f"  Face Processor (PID: {proc.info['pid']}) ✅"
                    )
                elif "config/" in cmdline:
                    config_name = [
                        part for part in proc.info["cmdline"] if "config/" in part
                    ][0]
                    running_machines.append(
                        f"  {config_name} (PID: {proc.info['pid']}) ✅"
                    )
...
def cmd_machine_health(args):
    ...
    try:
        ...
        total_jobs = sum(
            len(job_model.list_jobs(status=status))
...
def cmd_controller_log(args):
    ...
    try:
        ...
        for row in rows:
            ...
            job_id = (
                row["job_id"][:20] + "..." if len(row["job_id"]) > 23 else row["job_id"]
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Database CLI for state machine engine"
    )
    ...
    cleanup_parser.add_argument(
        ...
        choices=["pending", "processing", "completed", "failed"],
        required=True,
        help="Status of jobs to clean up",
    )

    # Reset processing command
    reset_processing_parser = subparsers.add_parser(
        "reset-processing", help="Reset stuck processing jobs to pending"
    )
    reset_processing_parser.add_argument(
        "--force",
        action="store_true",
    ...
    cleanup_events_parser = subparsers.add_parser(
        "cleanup-events", help="Clean up machine events"
    )
    cleanup_events_parser.add_argument(
        "--status",
    ...
    add_job_parser = subparsers.add_parser(
        "add-job", help="Add a new job to the database"
    )
    ...
    add_job_parser.add_argument(
        "--machine-type", help="Target machine type (defaults to job type)"
    )
    add_job_parser.add_argument("--input-file", help="Path to input file (optional)")
    add_job_parser.add_argument(
        "--payload", help="JSON payload with job-specific data (optional)"
    )

    # Complete job command
    complete_job_parser = subparsers.add_parser(
        "complete-job", help="Mark a job as completed"
    )
    ...
    # Remove job command
    remove_job_parser = subparsers.add_parser(
        "remove-job", help="Remove a job from the database"
    )
    ...
    recreate_parser = subparsers.add_parser(
        "recreate-database",
        help="Recreate database with fresh unified schema (DESTRUCTIVE)",
    )
    ...
    # Send event command
    send_event_parser = subparsers.add_parser(
        "send-event", help="Send an event to a target state machine"
    )
    ...
    send_event_parser.add_argument(
        "--source", help='Source machine name for UI display (defaults to "cli")'
    )
    send_event_parser.add_argument(
        "--job-id", dest="job_id", help="Related job ID (optional)"
    )
    send_event_parser.add_argument(
        "--payload", help="JSON payload for the event (optional)"
    )

    # List events command
    list_events_parser = subparsers.add_parser(
        "list-events", help="List machine events"
    )
    list_events_parser.add_argument("--target", help="Filter by target machine")
    list_events_parser.add_argument(
        "--status", choices=["pending", "processed"], help="Filter by event status"
    )
    list_events_parser.add_argument(
        "--limit", type=int, default=20, help="Limit number of results"
    )
    ...
    process_events_parser = subparsers.add_parser(
        "process-events", help="Show pending events for a machine"
    )
    process_events_parser.add_argument(
        "machine", help="Machine name to check events for"
    )

    # Machine status command
    machine_status_parser = subparsers.add_parser(
        "machine-status", help="Show concurrent machine status"
    )
    machine_status_parser.add_argument(
        "--machine", help="Filter by specific machine name"
    )
    ...
    machine_state_parser = subparsers.add_parser(
        "machine-state", help="Show current state of all machines"
    )
    ...
    transition_history_parser = subparsers.add_parser(
        "transition-history", help="Show state transition history"
    )
    transition_history_parser.add_argument("--machine", help="Filter by machine name")
    transition_history_parser.add_argument(
        "--hours", type=int, help="Show transitions from last N hours"
    )
    transition_history_parser.add_argument(
    ...
    error_history_parser = subparsers.add_parser(
        "error-history", help="Show error/exception history"
    )
    error_history_parser.add_argument("--machine", help="Filter by machine name")
    error_history_parser.add_argument(
        "--hours", type=int, help="Show errors from last N hours"
    )
    error_history_parser.add_argument(
        "--limit", type=int, default=20, help="Limit number of results (default: 20)"
    )
    ...
    controller_log_parser = subparsers.add_parser(
        "controller-log", help="Show controller event processing log"
    )
    controller_log_parser.add_argument(
        "--limit", type=int, default=20, help="Limit number of results (default: 20)"
    )

    # List errors command
    list_errors_parser = subparsers.add_parser(
        "list-errors", help="List error events and failed jobs for UI activity log"
    )
    list_errors_parser.add_argument(
        "--limit", type=int, default=50, help="Limit number of results (default: 50)"
    )
    list_errors_parser.add_argument(
        "--format",
        choices=["table", "json"],
