...
class WaitForJobsAction(BaseAction):
    ...
    async def execute(self, context: Dict[str, Any]) -> str:
        ...
        if not job_ids:
            logger.warning(
                f"[{machine_name}] No jobs tracked in context key '{self.tracked_jobs_key}'"
            )
        ...
        if "wait_start_time" not in context:
            ...
            logger.info(
                f"[{machine_name}] Starting to wait for {len(job_ids)} jobs: {job_ids}"
            )
        ...
        if elapsed > self.timeout:
            logger.warning(
                f"[{machine_name}] Timeout reached after {elapsed:.1f}s waiting for jobs"
            )
