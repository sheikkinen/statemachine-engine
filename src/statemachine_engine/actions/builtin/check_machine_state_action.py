...
class CheckMachineStateAction(BaseAction):
    ...
    async def execute(self, context: dict[str, Any]) -> str:
        ...
        try:
            ...
            if not is_running:
                logger.warning(
                    f"[{machine_name}] Machine {self.target_machine} process not found"
                )
                return "not_running"
            ...
            if not current_state:
                logger.warning(
                    f"[{machine_name}] No recent state data for {self.target_machine}"
                )
                return "not_running"
            ...
            if current_state in self.expected_states:
                logger.info(
                    f"[{machine_name}] Machine {self.target_machine} in expected state: {current_state}"
                )
                return "in_expected_state"
            else:
                logger.info(
                    f"[{machine_name}] Machine {self.target_machine} in unexpected state: {current_state} (expected: {self.expected_states})"
                )
    ...
    async def _is_process_running(self, machine_name: str) -> bool:
        ...
        try:
            ...
            is_running = "state_machine/cli.py" in output and machine_name in output

            logger.debug(
                f"Process check for {machine_name}: {'running' if is_running else 'not found'}"
            )
            return is_running

        except asyncio.TimeoutError:
            logger.error(
                "Process check timed out"
            )  # No context available in helper method
            return False
        except Exception as e:
            logger.error(
    ...
    def _get_current_state(self, machine_name: str) -> Optional[str]:
        ...
        try:
            with self.job_model.db._get_connection() as conn:
                ...
                if not row:
                    logger.debug(
                        f"No state data found for {machine_name}"
                    )  # Keep without prefix - debug level
                    return None
                ...
                logger.debug(
                    f"State timestamp for {machine_name}: completed_at={completed_at_str}, parsed={state_time}, now={now}, age={age:.1f}s"
                )  # Keep without prefix - debug level

                if age > self.timeout_seconds:
                    logger.warning(
                        f"State data for {machine_name} is stale ({age:.0f}s old, limit {self.timeout_seconds}s)"
                    )  # Keep without prefix - internal helper
                    return None

                state = row["current_state"]
                logger.debug(
                    f"Current state for {machine_name}: {state} (age: {age:.1f}s)"
                )  # Keep without prefix - debug level
                return state

        except Exception as e:
            logger.error(
                f"Error querying state for {machine_name}: {e}"
            )  # Keep without prefix - internal helper
            return None
