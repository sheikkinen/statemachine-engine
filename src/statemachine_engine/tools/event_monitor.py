...
class EventMonitor:
    ...
    async def run(self):
        ...
        if self.output_format != "json":
            ...
            if self.filter_machine:
                print(
                    f"🔍 Filtering events for machine: {self.filter_machine}",
                    file=sys.stderr,
                )
            if self.duration:
...
Examples:
  ...
  statemachine-events --host 192.168.1.10   Connect to remote server
    ...
    parser.add_argument("--machine", help="Filter events by machine name")
    ...
    parser.add_argument(
        "--duration", type=float, help="Monitor for N seconds then exit"
    )
    ...
    parser.add_argument(
        "--port", type=int, default=3002, help="WebSocket server port (default: 3002)"
    )
